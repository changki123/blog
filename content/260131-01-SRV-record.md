...
SRV(Service) 레코드는 특정 서비스의 위치 정보를 제공하는 DNS 레코드 타입임. 일반적인 A 레코드가 도메인을 IP 주소로 연결한다면, SRV 레코드는 서비스의 호스트명과 포트 정보까지 함께 제공할 수 있음.

<!-- more -->

## 왜 Minecraft 서버에서 SRV 레코드를 사용할까?

Minecraft Java Edition 서버는 기본적으로 25565 포트를 사용함. 일반적으로 서버에 접속하려면 다음과 같이 입력해야 함:
```
mc.example.com:25565
```

하지만 SRV 레코드를 설정하면 포트 번호 없이 깔끔하게 접속 가능:
```
play.example.com
```
...
이제 사용자는 `play.example.com`만 입력하면 자동으로 `mc.example.com:25565`로 연결됨.
...
사용자는 `survival.example.com` 입력만으로 포트 25570으로 자동 접속됨.
...
Cloudflare DNS 관리 페이지에서 다음과 같이 설정:
...
**주의사항**: Cloudflare의 프록시 기능(주황색 구름)은 SRV 레코드에 적용되지 않으므로 회색(DNS only)으로 설정됨.
...
SRV 레코드 설정 후 전파까지 최대 24-48시간이 걸릴 수 있음. TTL을 낮게 설정하면(300초 권장) 변경사항이 더 빠르게 반영됨.

### 여러 서버 간 부하 분산

동일한 Priority에서 Weight 값을 조정하여 트래픽을 분산할 수 있음:
```
...
이 경우 약 70%는 mc1으로, 30%는 mc2로 분산됨.
...
SRV 레코드를 활용하면 Minecraft 서버를 더 전문적이고 사용자 친화적으로 운영할 수 있음. 특히 여러 서버를 운영하거나 비표준 포트를 사용하는 경우 필수적인 설정임.

실제 운영 중인 AWS EC2 Minecraft 서버에 적용해보고, 더 나은 게임 경험을 만들어보자!