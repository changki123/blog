...
{{ figure(src="https://github.com/user-attachments/assets/5bfbe04d-4fa5-4bf5-87ab-6c8cc4a31e89", alt="Server Manager - Add Roles and Features", caption="terminimal 테마 이미지용 shortcode 테스트") }}
...
Active Directory Domain Service 체크 - Add Feature  <br>
...
Restart the destination server automatically if required 체크 - Yes - Install  <br>
...
Server Manager - Notifications! 클릭 - Promote this server to a domain controller 클릭  <br>

<img width="757" height="556" alt="Image" src="https://github.com/user-attachments/assets/ebdd1bb6-a3aa-4165-b5e2-48e447dbff3d" />
Add a new forest - test.com[원하는 도메인] 기입  <br>
...
사용자 로그인시 TEST\Administrator 로 접속 필요 (비밀번호는 바뀐거 없음)  <br>
...
Ethernet 3 더블클릭 - Properties - Internet Protocol Version 4 (TCP/IPv4) - Properties - Use the following DNS server addresses 선택 - AD IP 입력

---
## HOST이름 변경 및 도메인 가입

 시스템창 Win + Pause break

<img width="945" height="467" alt="Image" src="https://github.com/user-attachments/assets/635aae6b-b4e9-40e4-bf9e-ba8799806c91" />
System protection - Change… - Computer name 디비0102 변경 - Domain 클릭 - test.com 입력
...
Restart Now
DB01 02 세팅 완료, 이제 AD 계정 으로 진입.

또한 DB01 , DB02 TEST\sql_test 계정으로 진행

---
## Failover Cluster 설치

Server Manager - Mange - Add Roles and Feature
...
<img width="663" height="450" alt="Image" src="https://github.com/user-attachments/assets/702c65a1-8893-4813-8514-cc42e1bd5a61" />
2개 연결확인 (클러스터 조회가 안되는 경우 윈도우 버전 호환되는지 확인이 필요합니다.)

클러스터 지정시 각 DB로 통신은 되는데 조회가 안되는 이슈가 있을 경우 수동으로도 지정 가능합니다.

```
New-Cluster -Name CLU -Node MSSQLDB01, MSSQLDB02 -StaticAddress 172.31.30.100 -NoStorage -Force
이게 외됌;;
```
...
TinypingDB에 이제 (Synchronized) 생성된것 확인

## AlwaysOn 리스너 설정
AWS 에서 NLB 사용을 하는 경우 해당 설정이 필요없습니다.
...
<img width="702" height="657" alt="Image" src="https://github.com/user-attachments/assets/ab94c44f-9d5c-4c65-8b37-453b709016b8" />
Listener DNS Name : sql_lsn
...
<img width="704" height="657" alt="Image" src="https://github.com/user-attachments/assets/75fee148-626c-4080-b76f-8ae0227b75f4" />
리스너 전용 IP 추가 

IPv4 Address : 172.31.30.101

<img width="269" height="180" alt="Image" src="https://github.com/user-attachments/assets/db327b04-7257-4e51-bdc9-4dc80ace5ff6" />
리스너 추가 완료됨

<img width="1038" height="803" alt="Image" src="https://github.com/user-attachments/assets/e487582e-2c1b-484f-b1e6-fc4cfc920d7f" />
Failover Cluster Manager - FACLU.test.com - Roles - Resource에서 Server Name, IP Address 확인.

## AlwaysOn Failover 테스트 (DB01)
<img width="326" height="398" alt="Image" src="https://github.com/user-attachments/assets/53465b76-551c-48e9-b15f-e3298086c99b" />
Availability Groups - sql_ag 우클릭 - Failover…
...
sql_ag 우클릭 - Show Dashboard - Primary instance: 에서 DB02로 된것 확인 하면 완료

## Failback ( DB01, DB02 둘 중 한 곳에서 진행 )
동일하게 Failover… 클릭
...
Active Directory Users and Computers - test.com - Computers

<img width="756" height="614" alt="Image" src="https://github.com/user-attachments/assets/69cd9a22-6a59-4ffd-ac0c-7421ad8ec4b3" />
Active Directory Users and Computers - test.com - Users

## ++ AWS Failover/Failback 불가 이슈로 인한 IP추가
클러스터 자동 추가가 되지않아 명령어로 강제로 Failover Clouster 를 붙인경우가 이런경우
< 증거물 >
...
Secondary 대상으로 Listener IP 추가

<img width="969" height="298" alt="Image" src="https://github.com/user-attachments/assets/6aa13f80-1c07-4111-9056-232827b55361" />
예시로 172.31.20.100이 Cluster IP, 172.31.20.102가 AG Listener IP일 경우 이렇게 설정 (이 경우는 failback)

그래서

---

- Failover (MSSQLDB01 → MSSQLDB02)
1. SSMS에서 MSSQLDB02로 페일오버 실행
2. AWS 콘솔 → EC2 → Network Interfaces
3. MSSQLDB01 ENI에서 `172.31.20.102` 제거
4. MSSQLDB02 ENI에 `172.31.20.102` 할당

---

- Failback (MSSQLDB02 → MSSQLDB01)
1. SSMS에서 MSSQLDB01로 페일오버 실행
2. AWS 콘솔 → EC2 → Network Interfaces
3. MSSQLDB02 ENI에서 `172.31.20.102` 제거
4. MSSQLDB01 ENI에 `172.31.20.102` 할당

---

Failover 발생시! ENI 에서 옮겨주면 된다!!

## ++ 자동 Failover를 위해서 NLB 사용

NLB Primary 감지를 위해 shell 작성
...
while ($true) {
    try {
        $result = Invoke-Sqlcmd -Query "SELECT role_desc FROM sys.dm_hadr_availability_replica_states r JOIN sys.availability_replicas ar ON r.replica_id = ar.replica_id WHERE is_local = 1" -ServerInstance "localhost"
        
        if ($result.role_desc -eq "PRIMARY") {
            if ($null -eq $listener) {
...
서비스 등록
```
$action = New-ScheduledTaskAction -Execute "PowerShell.exe" -Argument "-ExecutionPolicy Bypass -File C:\Users\mscs\Desktop\healthcheck.ps1"
$trigger = New-ScheduledTaskTrigger -AtStartup
Register-ScheduledTask -TaskName "SQLHealthCheck" -Action $action -Trigger $trigger -RunLevel Highest -Force
Start-ScheduledTask -TaskName "SQLHealthCheck"
```

서비스 시작
```
Stop-ScheduledTask -TaskName "SQLHealthCheck"
# 스크립트 교체 후
Start-ScheduledTask -TaskName "SQLHealthCheck"
```

각서버 상태확인하면 서로 다른 결과가 나와야함
```
PS C:\Windows\system32> netstat -an | findstr "59999.*LISTEN"
  TCP    0.0.0.0:59999          0.0.0.0:0              LISTENING
```

> **%%% 만약 적용한다하면 어플리케이션에서 리스너 IP가 아닌! NLB DNS를 바라봐야 함. %%%**
